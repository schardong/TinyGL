...
CIExyzMesh::CIExyzMesh(std::vector<glm::vec3> xyz, std::vector<glm::vec3> rgb)
{
  std::vector<GLfloat> vertices((xyz.size() + 1) * 3); //+1*3 for the white point.
  ...
  for(size_t i = 0; i < colors.size() - 3; i += 3) {
    colors[i] = rgb[i / 3].x;
    colors[i + 1] = rgb[i / 3].y;
    colors[i + 2] = rgb[i / 3].z;
  }

  colors[colors.size() - 3] = 1.f;
  colors[colors.size() - 2] = 1.f;
  colors[colors.size() - 1] = 1.f;
